...
For podman and kiwi container builds, it sets a timestamp for the container build.
This can be a number (seconds since the epoch), the string `scm` (read time from
...
* rpmdb-source-date-epoch[:EPOCH]

Installs the build environment with the `$SOURCE_DATE_EPOCH` environment
variable set for rpm to override INSTALLTIME and INSTALLTID tags in the rpm
datbase. This makes the rpm database of the build environment reproducible.
EPOCH is a number (seconds since the epoch) and defaults to 0.
...
==== IgnoreRebuild: PACKAGE_OR_DEPENDENCY
