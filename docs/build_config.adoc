...
If a preinstalled package ships /usr/lib/build/rpmdb-source-date-epoch,
the build environment is installed with SOURCE_DATE_EPOCH set to the file
content (empty means 0), so rpm >= 4.18 creates a reproducible rpm database.
